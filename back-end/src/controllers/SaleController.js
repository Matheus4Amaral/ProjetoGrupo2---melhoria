...
export const deleteSale = async (req, res) => {
  ...
  try {
    ...
    await client.query("DELETE FROM public.possui_venda WHERE id_venda = $1", [
      id,
    ]);
    ...
    await client.query("DELETE FROM public.venda WHERE id_venda = $1", [id]);
...
export const incrementProduct = async (req, res) => {
  ...
  try {
    ...
    if (checkProduct.rows.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "Produto não encontrado nesta venda" });
    }
...
export const updateSale = async (req, res) => {
  ...
  if (valor_venda !== undefined && valor_venda < 0) {
    return res
      .status(400)
      .json({ message: "Valor da venda não pode ser negativo" });
  }
  ...
  try {
    ...
    const novaDescricao =
      descricao !== undefined ? descricao : vendaAtual.descricao;
    const novaData =
      data_venda !== undefined ? data_venda : vendaAtual.data_venda;
    const novoValor =
      valor_venda !== undefined ? valor_venda : vendaAtual.valor_venda;
...
export const addProductToSale = async (req, res) => {
  ...
  if (!id_produto || !quantidade_venda) {
    return res
      .status(400)
      .json({ message: "id_produto e quantidade_venda são obrigatórios" });
  }
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "Produto adicionado à venda com sucesso" });
  } catch (error) {
    await client.query("ROLLBACK");
    return res.status(500).json({
    ...
    if (produtos && Array.isArray(produtos)) {
      for (const item of produtos) {
        ...
        if (!id_produto || !quantidade_venda || quantidade_venda <= 0) {
          ...
          return res.status(400).json({
            message:
              "Cada produto precisa ter id_produto e quantidade_venda válidos",
          });
        }
