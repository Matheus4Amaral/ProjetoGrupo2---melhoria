...
export const getStockByNome = async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Nenhum estoque encontrado com esse nome" });
    }
...
export const updateStock = async (req, res) => {
  try {
    ...
    const novaDescricao =
      descricao !== undefined ? descricao : estoqueAtual.descricao;
...
export const addProductToStock = async (req, res) => {
  try {
    ...
    const { id_produto, quantidade_estoque_total, quantidade_estoque_atual } =
      req.body;
    const decoded = getUserFromToken(req);
    const id_usuario = decoded.id_usuario;
    ...
    if (
      !id_produto ||
      quantidade_estoque_total === undefined ||
      quantidade_estoque_atual === undefined
    ) {
      return res.status(400).json({
        message:
          "id_produto, quantidade_estoque_total e quantidade_estoque_atual são obrigatórios",
      });
    }
    ...
    if (quantidade_estoque_total < 0 || quantidade_estoque_atual < 0) {
      return res
        .status(400)
        .json({ message: "Quantidades não podem ser negativas" });
    }
    ...
    if (checkProduct.rows.length > 0) {
      return res
        .status(400)
        .json({ message: "Produto já existe neste estoque" });
    }
    ...
    return res
      .status(201)
      .json({ message: "Produto adicionado ao estoque com sucesso" });
  } catch (error) {
    return res.status(500).json({
...
export const updateProductInStock = async (req, res) => {
  try {
    ...
    if (checkProduct.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Produto não encontrado neste estoque" });
    }
    ...
    const novaQuantidadeTotal =
      quantidade_estoque_total !== undefined
        ? quantidade_estoque_total
        : produtoAtual.quantidade_estoque_total;
    const novaQuantidadeAtual =
      quantidade_estoque_atual !== undefined
        ? quantidade_estoque_atual
        : produtoAtual.quantidade_estoque_atual;
...
export const removeProductFromStock = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Produto removido do estoque com sucesso" });
  } catch (error) {
    return res.status(500).json({
