...
export const createProduct = async (req, res) => {
  ...
  if (nome_produto.length > 255) {
    return res
      .status(400)
      .json({ message: "Nome do produto não pode ter mais de 255 caracteres" });
  }
  ...
  if (
    ...
    preco_compra === null ||
    Number(preco_compra) <= 0
  ) {
    return res
      .status(400)
      .json({ message: "Preço de compra deve ser maior que zero" });
  }
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Preço de venda deve ser maior que zero" });
  }

  if (
    quantidade_inicial === undefined ||
    quantidade_inicial === null ||
    Number(quantidade_inicial) < 0
  ) {
    return res
      .status(400)
      .json({
        message: "Quantidade inicial é obrigatória e não pode ser negativa",
      });
  }
  ...
  try {
    ...
    if (
      ...
      if (fornecedorResult.rows.length === 0) {
        ...
        return res
          .status(403)
          .json({ message: "Fornecedor não encontrado para este usuário" });
      }
    }
    ...
    const resultProduct = await client.query(
      ...
      [
        ...
        volume === undefined || volume === null || volume === ""
          ? null
          : Number(volume),
        descricao === undefined || descricao === null || descricao.trim() === ""
          ? null
          : descricao.trim(),
        lote === undefined || lote === null || lote === ""
          ? null
          : Number(lote),
        Number(preco_venda),
        id_usuario,
...
export const updateProduct = async (req, res) => {
  ...
  if (
    nome_produto !== undefined &&
    (nome_produto.trim() === "" || nome_produto.length > 255)
  ) {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [
        ...
        id_fornecedor !== undefined
          ? id_fornecedor === null || id_fornecedor === ""
            ? null
            : Number(id_fornecedor)
